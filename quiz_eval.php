...
session_start();
 ...
 $result1=mysqli_query($con,"SELECT * FROM user_answers WHERE TYPEID='$typeid' AND EMAIL='$email' ") or die("Could not execute query: " .mysqli_error($con));
 ...
 while ($number_of_qns>=$qid) {
 	...
 	$result4=mysqli_query($con,"SELECT * FROM user_answers WHERE TYPEID='$typeid' AND QID='$qid' AND EMAIL='$email' ");
 	...
 	if($count1!=0){
 	   ...
 	   if ($ans==$user_ans) {
 		  $_SESSION['score']++;
 	   	  ...
 	   	  $a = ++$_SESSION['wqid'];
  		  $wrong_qns1=mysqli_query($con,"INSERT INTO wrong(TYPEID,WQID,QID,EMAIL) VALUES ('$typeid','$a','$qid','$email') ");	
 		 
 	   }
    }else{
    	$_SESSION['wrong_or_unanswered']++;
    	$b = ++$_SESSION['wqid'];
  		$wrong_qns2=mysqli_query($con,"INSERT INTO wrong(TYPEID,WQID,QID,EMAIL) VALUES ('$typeid','$b','$qid','$email') ");	
    }
    $qid++;
